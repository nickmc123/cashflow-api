...
CASAXAI_API_KEY = os.environ.get("CASAXAI_API_KEY", "casaxai-secret-key-2026")
...
SPECIAL_TRANSACTIONS = {
    ...
    "2026-02-17": [
        {"type": "payroll", "amount": -60000, "desc": "Payroll Checks"},
        {"type": "amex", "amount": -83000, "desc": "AmEx Payment #1"},
        {"type": "amex", "amount": -57000, "desc": "AmEx Payment #2"},
    ],
    ...
    "2026-03-09": [{"type": "income", "amount": 78000, "desc": "Cert Sales CC Deposit (Charged 3/2, Posts Monday)"}],
    "2026-03-16": [
        {"type": "comms_execs", "amount": -46000, "desc": "Comms & Execs"},
        {"type": "payroll_tax", "amount": -25430, "desc": "ADP Tax + 401K + Fees"},
    ],
...
async def get_summary(code: str = Query(...)):
    ...
    # Only consider weekdays to avoid weekend plateaus
    rows = proj["rows"]
    ...
    # Find local highs and lows among weekdays
    ...
    # Also include absolute min and max among weekdays if not already there
    min_row = min(weekday_rows, key=lambda x: x["balance"])
    max_row = max(weekday_rows, key=lambda x: x["balance"])
...
• Feb 17: Payroll Checks (~$60K)"""
    ...
    if 'payment' in q or 'due' in q or 'coming up' in q or 'upcoming' in q:
        ...
        payments = [
            ...
            {"date": "2026-03-17", "desc": "AmEx ($83K+$28K JL) + Payroll", "amount": 171000},
